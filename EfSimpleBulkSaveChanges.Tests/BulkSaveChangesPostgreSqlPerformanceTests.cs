...
public sealed class BulkSaveChangesPostgreSqlPerformanceTests
{
    ...
    public async Task Synchronize_PerformanceAcrossSaveChangesAndBulkBatchSizes()
    {
        ...
        foreach (var rowCount in RowCounts)
        {
            IReadOnlyList<User> saveSourceUsers = [];

            var saveChangesMeasurement = await MeasureAsync(
                "synchronize",
                "SaveChanges",
                rowCount,
                null,
                ...
                async db =>
                {
                    ...
                    saveSourceUsers = await CreateSynchronizeSourceAsync(db, rowCount, "save-sync");
                    return GetSynchronizeSavedCount(rowCount);
                },
                async db =>
                {
                    await StageManualSynchronizeAsync(db, saveSourceUsers);
    ...
    private static async Task StageManualSynchronizeAsync(
        PerformanceDbContext db,
        IReadOnlyList<User> sourceUsers)
    {
        var users = await db.Users.OrderBy(user => user.Id).ToListAsync();
        var usersById = users.ToDictionary(user => user.Id);
        var sourceKeys = sourceUsers
        ...
        {
            if (sourceUser.Id != 0 && usersById.TryGetValue(sourceUser.Id, out var existingUser))
            {
                existingUser.FirstName = sourceUser.FirstName;
                existingUser.LastName = sourceUser.LastName;
                existingUser.Email = sourceUser.Email;
                continue;
            }
        ...
        foreach (var user in users)
        {
            if (!sourceKeys.Contains(user.Id))
            {
